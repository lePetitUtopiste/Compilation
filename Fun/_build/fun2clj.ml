...
let translate_program e =
  ...
  let rec tr_expr (e: Fun.expression) (bvars: VSet.t):
    ...
    and crawl e bvars = match e with
      ...
      | Fun.LetRec(x,Fun(s1,e1), e2) ->
        let bvars2 = VSet.add x bvars in
        let suite = crawl e2 bvars2 in
        let n = new_fname () in
        let code, trucmuche = tr_expr e1 ((VSet.singleton s1)) in
        let f = {Clj.name = n; Clj.code = code ; Clj.param = s1} in
        fdefs:= f::!fdefs;
        let rec extrait cvars = match cvars with
        | (x,_)::xs -> Clj.Var(Clj.Name(x))::extrait xs
        | [] -> []
        in
        Clj.LetRec( x,Clj.Tpl((Clj.FunRef(n))::(extrait trucmuche)), suite)
